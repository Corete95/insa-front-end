...
describe("Navigation", () => {
  ...
  const monthViewArrows = monthViewComponent.find(
  ...
  it("jumps 12 months back on prev2 button click for month view", () => {
    ...
    expect(monthSetActiveStartDateFn).toHaveBeenCalledWith(
      new Date(2016, 0, 1)
    );
  });
  ...
  const yearViewArrows = yearViewComponent.find(
    "button.react-calendar__navigation__arrow"
  );
  ...
  const decadeViewArrows = decadeViewComponent.find(
    "button.react-calendar__navigation__arrow"
  );
  ...
  it("jumps 10 decades back on prev2 button click for decade view", () => {
    ...
    expect(decadeSetActiveStartDateFn).toHaveBeenCalledWith(
      new Date(1911, 0, 1)
    );
  });
  ...
  const centuryViewArrows = centuryViewComponent.find(
    "button.react-calendar__navigation__arrow"
  );
  ...
  it("jumps 1 century back on prev button click for century view", () => {
    ...
    expect(centurySetActiveStartDateFn).toHaveBeenCalledWith(
      new Date(1901, 0, 1)
    );
  });
  ...
  describe("formats navigation label properly", () => {
    it("displays calendar with custom month year navigation label", () => {
      ...
      const navigationLabel = component
